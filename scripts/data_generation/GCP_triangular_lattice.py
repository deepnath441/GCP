...
for ensemble in range(1000):
    ...
    for probability in range(100):
        ...
        #print(theta_in_pi)
        #p=2.0*((math.sin(theta_in_pi))**2.0)
        c=math.sin(2.0*theta_in_pi)
        #print(c)
        G_classical=nx.Graph()
        #G_classical=nx.complete_graph(N)
        G_classical=nx.grid_graph(dim=[N,N])
        
        for i in G_classical.nodes:
            temp_0=i[0]+1
            temp_1=i[1]+1
            for j in G_classical.nodes:
                if temp_0==j[0] and temp_1==j[1]:
                    G_classical.add_edge(i,j)
                else:
                    pass
        #print(G_classical.edges)
        #nx.draw(G_classical, with_labels=True)
        #plt.show()
        #break
        ...
        for node_pair in node_pair_list_quantum:
            ...
            if nx.has_path(G_classical,node_pair[0],node_pair[1])==True: # if there is a classical path - there will be a quantum edge
                ...
                if node_pair[0][0]<node_pair[1][0]:
                    ...
                    y_t=node_pair[1][1]
                    if y_s>=y_t:
                        numerator=abs(x_s-x_t)+abs(y_s-y_t)
                        denominator=abs(x_s-x_t)
                    else:
                        diff_x_s_x_t=abs(x_s-x_t)
                        diff_y_s_y_t=abs(y_s-y_t)
                        numerator_1=min(diff_x_s_x_t,diff_y_s_y_t)
                        numerator_2=abs(abs(x_s-x_t)-abs(y_s-y_t))
                ...
                number_of_shortest_path=comb(int(numerator),int(denominator))
                shortest_path_length=numerator # for all shortest paths - path length will be same - shortest
                c_for_parallel_paths=[]
                for j in range(int(number_of_shortest_path)): # calcualte probability by series rule for each shortest path
